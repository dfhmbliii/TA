...
@section('title', 'Laporan Analisis SPK')
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadData() {
        ...
        try {
            ...
            const response = await fetch(`{{ route('reports.spk-analysis.data') }}?${params}`);
    ...
    window.showDetail = async function(resultId) {
        try {
            const response = await fetch(`{{ route('reports.spk-analysis.detail', ':id') }}`.replace(':id', resultId));
